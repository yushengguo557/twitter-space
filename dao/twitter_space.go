...
func SaveTwitterSpace(space models.TwitterSpace) (err error) {
	...
	resp := global.App.DB.Model(&models.TwitterSpace{}).Where("id = ?", space.ID).Find(&sps)
	if err = resp.Error; err != nil {
		return err
	}
	if resp.RowsAffected > 0 {
		log.Println("Update Space: ", space.ID)
		if err = global.App.DB.Model(&models.TwitterSpace{}).Where("id = ?", space.ID).Updates(&space).Error; err != nil {
		...
		if err = global.App.DB.Model(&models.TwitterSpace{}).Create(&space).Error; err != nil {
